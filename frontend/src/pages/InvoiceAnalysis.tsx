  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col h-[800px] border border-slate-800 rounded-xl overflow-hidden bg-slate-900/40">
          <div className="p-3 border-b border-slate-800 bg-slate-900/60 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-200">Invoice Document</h2>
            <a 
          ...
          <iframe 
            src={`${invoice.file_path}#view=FitH`} 
            className="flex-1 w-full bg-slate-950" 
            title="Invoice PDF"
          />
        </div>

        {/* Right Column: Analysis & Charges */}
        <div className="space-y-6">
          {analysed && (
          ...
          {!isClient && (
            <div className="p-4 rounded-lg border border-slate-700 bg-slate-900/40">
              <p className="text-sm text-slate-400">
                Invoice analysis is available to the client. You can see the extracted charge lines below.
              </p>
            </div>
          )}
            ...
            <h2 className="text-base font-semibold text-slate-200">
              ...
              {isClient && (
                <span className="ml-2 text-xs font-normal text-slate-500">
                  — {analysed && quoteDetail ? 'compared against quote amounts' : 'mapped to your Charge Master'}
                </span>
              )}
            </h2>
            <ChargeLineTable
              ...
              anomalies={anomalies}
              quoteCharges={analysed && isClient ? (quoteDetail?.charges ?? []) : []}
            />
          </div>
        </div>
      </div>
